...
pub fn to_dot(problem: &Problem) -> String {
    ...
    writeln!(&mut dot, "    node [shape=box, style=filled, fillcolor=white];").unwrap();
    ...
    for domain in &problem.domains {
        ...
        writeln!(&mut dot, "    \"{}\" [label=\"{}\", shape={}, fillcolor={}];", domain.name, label, shape, color).unwrap();
    }
    ...
    for req in &problem.requirements {
         ...
         writeln!(&mut dot, "    \"{}\" [shape=note, style=dashed, label=\"{}\\n[{}]\"];", req.name, req.name, frame_label).unwrap();
         
         // Connect Requirement to Constrained/Referenced domains
         if !req.constrains.is_empty() {
             writeln!(&mut dot, "    \"{}\" -> \"{}\" [style=dashed, arrowheaders=none, label=\"constrains\"];", req.name, req.constrains).unwrap();
         }
         if !req.reference.is_empty() {
              writeln!(&mut dot, "    \"{}\" -> \"{}\" [style=dashed, arrowheaders=none, label=\"references\"];", req.name, req.reference).unwrap();
         }
    }
    ...
    for interface in &problem.interfaces {
        ...
        for phen in &interface.shared_phenomena {
            ...
            if !connections.contains(&pair) && !connections.contains(&(pair.1.clone(), pair.0.clone())) {
        ...
        if let Some((src, dst)) = connections.first() {
             ...
             writeln!(&mut dot, "    \"{}\" -> \"{}\" [dir=both, label=\"{}\"];", src, dst, label_str).unwrap();
        }
    }
